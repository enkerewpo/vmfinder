...
from vmfinder import __version__, __author__, __author_email__, __url__, __description__, __copyright__
...
from vmfinder.commands.vm import (
    cmd_vm_list, cmd_vm_start, cmd_vm_stop, cmd_vm_suspend, cmd_vm_resume, cmd_vm_restart,
    cmd_vm_create, cmd_vm_delete, cmd_vm_info,
    cmd_vm_set_cpu, cmd_vm_set_memory, cmd_vm_fix_permissions, cmd_vm_resize_disk,
    cmd_vm_console, cmd_vm_ssh, cmd_vm_set_password,
...
def create_parser():
    ...
    # init command
    parser_init = subparsers.add_parser('init', help='Initialize VMFinder with default templates')
    parser_init.set_defaults(func=cmd_init.cmd_init)
    
    # install-completion command
    parser_install_comp = subparsers.add_parser('install-completion', help='Install tab completion for bash and zsh')
    ...
    template_subparsers = parser_template.add_subparsers(dest='template_command', help='Template commands')
    
    parser_template_list = template_subparsers.add_parser('list', help='List all available templates')
    parser_template_list.set_defaults(func=cmd_template.cmd_template_list)
    
    parser_template_create = template_subparsers.add_parser('create', help='Create a new template')
    parser_template_create.add_argument('name', help='Template name')
    parser_template_create.add_argument('--os', required=True, help='Operating system name')
    ...
    parser_template_create.add_argument('--arch', default='x86_64', help='Architecture (default: x86_64)')
    parser_template_create.add_argument('--description', help='Template description')
    parser_template_create.add_argument('--cloud-image-url', help='Cloud image URL for auto-install')
    parser_template_create.add_argument('--cloud-image-support', action='store_true', help='Enable cloud image support')
    parser_template_create.add_argument('--no-cloud-image-support', dest='cloud_image_support', action='store_false', help='Disable cloud image support')
    ...
    # vm create
    parser_vm_create = vm_subparsers.add_parser('create', help='Create a new virtual machine')
    parser_vm_create.add_argument('name', help='VM name')
    template_arg = parser_vm_create.add_argument('--template', '-t', required=True, help='Template name')
    if ARGCOMPLETE_AVAILABLE:
        template_arg.completer = complete_template_name
    parser_vm_create.add_argument('--cpu', '-c', type=int, default=2, help='Number of CPUs (default: 2)')
    parser_vm_create.add_argument('--memory', '-m', type=int, default=2048, help='Memory in MB (default: 2048)')
    parser_vm_create.add_argument('--disk-size', '-d', type=int, default=20, help='Disk size in GB (default: 20)')
    network_arg = parser_vm_create.add_argument('--network', default='default', help='Network name (default: default)')
    if ARGCOMPLETE_AVAILABLE:
        network_arg.completer = complete_network_name
    parser_vm_create.add_argument('--auto-install', action='store_true', default=True, help='Automatically install OS from cloud image (default: enabled)')
    parser_vm_create.add_argument('--no-auto-install', dest='auto_install', action='store_false', help='Disable auto-install')
    parser_vm_create.add_argument('--force', '-f', action='store_true', help='Force overwrite existing VM without prompting')
    ...
    parser_vm_stop.add_argument('--force', '-f', action='store_true', help='Force stop (destroy)')
    parser_vm_stop.set_defaults(func=cmd_vm_stop)
    
    # vm suspend
    parser_vm_suspend = vm_subparsers.add_parser('suspend', help='Suspend a virtual machine')
    ...
    # vm resume
    parser_vm_resume = vm_subparsers.add_parser('resume', help='Resume a suspended virtual machine')
    ...
    # vm restart
    parser_vm_restart = vm_subparsers.add_parser('restart', help='Restart a virtual machine')
    vm_restart_name = parser_vm_restart.add_argument('name', help='VM name')
    if ARGCOMPLETE_AVAILABLE:
        vm_restart_name.completer = complete_vm_name
    parser_vm_restart.add_argument('--force', '-f', action='store_true', help='Force stop (destroy) before restart')
    parser_vm_restart.set_defaults(func=cmd_vm_restart)
    
    # vm delete
    parser_vm_delete = vm_subparsers.add_parser('delete', help='Delete a virtual machine')
    vm_delete_name = parser_vm_delete.add_argument('name', help='VM name')
    if ARGCOMPLETE_AVAILABLE:
        vm_delete_name.completer = complete_vm_name
    parser_vm_delete.add_argument('--delete-disk', action='store_true', help='Also delete the disk image')
    parser_vm_delete.add_argument('--yes', '-y', action='store_true', help='Skip confirmation prompt')
    parser_vm_delete.set_defaults(func=cmd_vm_delete)
    
    # vm info
    parser_vm_info = vm_subparsers.add_parser('info', help='Show detailed information about a virtual machine')
    ...
    # vm fix-cloud-init
    parser_vm_fix_cloud_init = vm_subparsers.add_parser('fix-cloud-init', help='Fix cloud-init metadata service warnings')
    vm_fix_ci_name = parser_vm_fix_cloud_init.add_argument('name', help='VM name')
    if ARGCOMPLETE_AVAILABLE:
        vm_fix_ci_name.completer = complete_vm_name
    parser_vm_fix_cloud_init.add_argument('--start', action='store_true', help='Start VM after fixing cloud-init')
    parser_vm_fix_cloud_init.add_argument('--no-start', dest='start', action='store_false', help='Do not start VM after fixing')
    parser_vm_fix_cloud_init.set_defaults(start=False, func=cmd_vm_fix_cloud_init)
    
    # vm set-cpu
    parser_vm_set_cpu = vm_subparsers.add_parser('set-cpu', help='Set CPU count for a virtual machine')
    ...
    # vm set-memory
    parser_vm_set_memory = vm_subparsers.add_parser('set-memory', help='Set memory for a virtual machine')
    ...
    # vm console
    parser_vm_console = vm_subparsers.add_parser('console', help='Show console command for a virtual machine')
    ...
    # vm set-password
    parser_vm_set_password = vm_subparsers.add_parser('set-password', help='Set password for a VM using cloud-init')
    vm_set_pwd_name = parser_vm_set_password.add_argument('name', help='VM name')
    if ARGCOMPLETE_AVAILABLE:
        vm_set_pwd_name.completer = complete_vm_name
    parser_vm_set_password.add_argument('--username', '-u', default='ubuntu', help='Username to set password for (default: ubuntu)')
    parser_vm_set_password.add_argument('--password', '-p', help='Password to set (prompts if not provided)')
    parser_vm_set_password.add_argument('--start', action='store_true', default=True, help='Start VM after setting password (default: start)')
    parser_vm_set_password.add_argument('--no-start', dest='start', action='store_false', help='Do not start VM after setting password')
    parser_vm_set_password.set_defaults(func=cmd_vm_set_password)
    
    # vm ssh
    parser_vm_ssh = vm_subparsers.add_parser('ssh', help='Show SSH connection information for a VM')
    vm_ssh_name = parser_vm_ssh.add_argument('name', help='VM name')
    if ARGCOMPLETE_AVAILABLE:
        vm_ssh_name.completer = complete_vm_name
    parser_vm_ssh.add_argument('--username', '-u', default='ubuntu', help='SSH username (default: ubuntu)')
    parser_vm_ssh.add_argument('--port', '-p', type=int, default=22, help='SSH port (default: 22)')
    key_arg = parser_vm_ssh.add_argument('--key', '-k', help='SSH private key file path')
    if ARGCOMPLETE_AVAILABLE:
        key_arg.completer = complete_file_path
    parser_vm_ssh.set_defaults(func=cmd_vm_ssh)
    
    # vm fix-permissions
    parser_vm_fix_permissions = vm_subparsers.add_parser('fix-permissions', help='Fix disk permissions for a VM')
    ...
    # vm resize-disk
    parser_vm_resize_disk = vm_subparsers.add_parser('resize-disk', help='Resize a VM\'s disk image file')
...
def main():
    ...
    # Setup logging
    log_level = 'DEBUG' if args.verbose else (args.log_level if hasattr(args, 'log_level') else 'INFO')
    if args.quiet:
        log_level = 'ERROR'
