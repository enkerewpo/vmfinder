...
class CloudImageManager:
    ...
    def create_disk_from_cloud_image(self, cloud_image_path: Path, 
                                      disk_path: Path, size_gb: int) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ['qemu-img', 'convert', '-f', 'qcow2', '-O', 'qcow2',
                 str(cloud_image_path), str(disk_path)],
                check=True,
                capture_output=True
            )
            ...
            if abs(size_gb - original_size_gb) > 0.1:  # Allow small floating point differences
                subprocess.run(
